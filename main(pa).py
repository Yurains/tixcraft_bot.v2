...
CONST_APP_VERSION = "TicketRobot v1.1(PaddleOCR)"
...
def load_translate():
    ...
    zh_tw["ocr_captcha_ddddocr_beta"] = 'Paddle OCR辨識'
...
class OcrHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        # 1) 解析 request body，確定有 image_data
        try:
            _body = json.loads(self.request.body)
        except Exception:
        ...
        try:
            ...
            self.write({"answer": "", "error": "base64 decode fail"})
            return

        # 3) 將二進位影像讀入 PIL，再轉為 numpy array
        if self.application.ocr is not None:
            try:
                pil_image = Image.open(io.BytesIO(img_base64))
                # 視情況轉成 RGB
                pil_image = pil_image.convert("RGB")

                # PaddleOCR 的 ocr() 通常傳入圖片路徑或 Numpy 陣列
                ocr_result = self.application.ocr.ocr(np.array(pil_image))

                # 4) 取得 ocr 結果 (每行是一個 list，中間帶文字、置信度等資訊)
                #    常見回傳格式: [ [ [x0,y0],[x1,y1],[x2,y2],[x3,y3] ], [("文字內容",confidence)] ]
                #    如果是一張圖包含多行，最外層會是一個list
                if ocr_result:
                    # 假設只想把所有行的文字拼起來
                    recognized_text_list = []
                    for line_data in ocr_result:
                        # line_data[1][0] 就是該行辨識後的文字
                        recognized_text_list.append(line_data[1][0])
                    ocr_answer = "".join(recognized_text_list)
            except Exception as exc:
                self.write({"answer": "", "error": str(exc)})
                return

        # 5) 回傳最終結果 (若想回傳多欄資訊也可以一起整合)
...
async def main_server():
    ...
    try:
        # 建立 PaddleOCR 實例
        # 常見參數範例: use_angle_cls=True, lang='ch', show_log=False
    ...
    # 把 OCR 實例掛到 tornado 的 app 之下，方便各 Handler 共用
    app.ocr = ocr_engine
